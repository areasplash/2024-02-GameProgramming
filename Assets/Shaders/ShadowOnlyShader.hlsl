#ifndef SHADOW_ONLY_SHADER_H
#define SHADOW_ONLY_SHADER_H

	struct ShadowOnlyData {
		float3 position;
		float4 rotation;
		float3 scale;
	};

	StructuredBuffer<ShadowOnlyData> _ShadowOnlyData;
	...
	inline void SetUnityMatrices(uint instanceID, inout float4x4 objectToWorld, inout float4x4 worldToObject) {
			...
			ShadowOnlyData data = _ShadowOnlyData[instanceID];
	...
	void Passthrough_float(in float3 In, out float3 Out) {
		Out = In;
		#ifdef UNITY_PROCEDURAL_INSTANCING_ENABLED
			ShadowOnlyData data = _ShadowOnlyData[unity_InstanceID];
		#endif
	}
